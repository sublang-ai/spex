...
A package is one file, one intent, one closed set of subjects ([[meta-14](specs/meta.md#meta-14)], [[meta-13](specs/meta.md#meta-13)]).
It stands alone: readable in full without following any link ([[meta-20](specs/meta.md#meta-20)]).

- **Rely on another package only by citing it, at the exact phrase that relies on it.**
  A citation is the only relationship between packages, and it binds a phrase to a specific External Behavior item of the cited package ([[meta-19](specs/meta.md#meta-19)], [[meta-25](specs/meta.md#meta-25)]).
...
- **Name a selectable counterparty abstractly, and let neither side name the other.**
  A fixed semantic dependency gets a citation; a replaceable counterparty stays an uncited general phrase, which is sufficient ([[meta-19](specs/meta.md#meta-19)]).
...
- **State the wiring as a composition package's own behavior, nowhere else.**
  A composition package is an ordinary package whose behavior emerges only when several packages work together; one phrase may cite several packages exactly when it composes their behaviors ([[meta-19](specs/meta.md#meta-19)]).
...
- **Make the boundary itself testable from both sides.**
  ...
  Each package verifies its own side against a double of the abstract counterparty — [[meta-23](specs/meta.md#meta-23)] prefers executing the real behavior of a *cited* package, and the counterparty is deliberately uncited.

## 2. Split external from internal behavior by audience

External behavior is what a package's users — humans, hosts, or peer packages — may rely on; internal behavior is implementation hidden from them ([[meta-15](specs/meta.md#meta-15)]).
No behavior item cites a peer's internal item ([[meta-19](specs/meta.md#meta-19)]): [[protected-content-2](specs/packages/protected-content.md#protected-content-2)] pins four independent gate layers naming the internal ones only generally, while its sweep test [[protected-content-3](specs/packages/protected-content.md#protected-content-3)] cites [[course-catalog-12](specs/packages/catalog/course-catalog.md#course-catalog-12)] and [[web-shell-6](specs/packages/site/web-shell.md#web-shell-6)] directly — a citation the meta law is silent on: it neither licenses nor forbids a test citing peer Internal Behavior, and the citation reclassifies nothing.
The split governs use, not verification: every stated behavior is verified unless verification is irrelevant to it, white-box or black-box ([[meta-24](specs/meta.md#meta-24)]).
...
- **The audience is per-package, and peers count as users.**
  ...
  [[github-login-9](specs/packages/identity/github-login.md#github-login-9)]'s verification guarantee is external because [[video-library-8](specs/packages/catalog/video-library.md#video-library-8)] relies on it — what a peer may rely on is External Behavior ([[meta-15](specs/meta.md#meta-15)], [[meta-19](specs/meta.md#meta-19)]).
- **State only behavior some test or inspection could distinguish** ([[meta-2](specs/meta.md#meta-2)], [[meta-24](specs/meta.md#meta-24)]).
...
- **One GEARS statement per item; attachments elaborate it** ([[meta-12](specs/meta.md#meta-12)]).
  ...
  If naming an item's purpose needs "and" — "asset listing and deletion" — inspect it for a split: differing stateful preconditions or triggers are evidence of additional items ([[meta-12](specs/meta.md#meta-12)]).
- **Decisions are not where implementation details live** ([[meta-7](specs/meta.md#meta-7)]).
...
- **Intents are self-contained prose with no citations** ([[meta-15](specs/meta.md#meta-15)], [[meta-20](specs/meta.md#meta-20)]).
...
- **Collections carry no semantics** ([[meta-21](specs/meta.md#meta-21)]).
  `packages/identity/` and `packages/catalog/` are shelves, not layers; a file's identity is its basename ([[meta-16](specs/meta.md#meta-16)]), so regrouping changes relative citation paths but never an ID, an anchor, or a meaning.
...
- **Each composition package is one integrated concern, and its tests are release evidence — acceptance journeys for seams users walk, inspections for hidden ones** ([[meta-24](specs/meta.md#meta-24)]).
...
- **A test cites every behavior it verifies, inline at the assertion that verifies it** ([[meta-28](specs/meta.md#meta-28)]).
  ...
  Items carry no relationship-metadata lines; the citations in an item's clauses are the single source of its relationships ([[meta-25](specs/meta.md#meta-25)]).
  And a test prefers executing the real behavior of a cited package to substituting it ([[meta-23](specs/meta.md#meta-23)]): the composition tests run the composed packages for real on seeded deployments, with a stub only for the outside vendor ([[lesson-playback-3](specs/packages/lesson-playback.md#lesson-playback-3)]'s stub GitHub provider).
...
- **A user-walked seam is External Behavior with acceptance tests.**
  ...
  The wiring behind it is Internal Behavior: [[course-publishing-1](specs/packages/course-publishing.md#course-publishing-1)] serves the catalog's media delegations through the video library, hidden from members and cited by the package's own tests at the assertions that verify it ([[meta-28](specs/meta.md#meta-28)]).
- **An inspection-only supply seam is verified by inspecting a deployment.**
  No product user observes which store holds a record, so [[platform-services-6](specs/packages/platform-services.md#platform-services-6)] inspects configuration and egress instead of walking a journey — verification may be white-box ([[meta-24](specs/meta.md#meta-24)]).
