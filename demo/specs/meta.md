...
elaborate that contract alone, including any format, grammar, or
definition the clause names:
...
second trigger, so [META-6](#meta-6)'s one-trigger rule governs
the clause and not its attachments; differing triggers or
lifecycles are strong evidence of a second contract rather than
proof of one; and an umbrella such as "handle correctly" or
"support behavior" names no contract.
