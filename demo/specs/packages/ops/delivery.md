...
A migration applied before cutover [[delivery-8](#delivery-8)] shall stay compatible with the still-serving revision, so a failure after migration leaves that revision serving correctly.
...
| --- | --- |
| a pull request carrying a failing check | the required checks are reported on it [[delivery-1](#delivery-1)], and merging it into the default branch is blocked [[delivery-2](#delivery-2)] |
| a pull request whose build passes | a preview deployment link appears on the pull request [[delivery-3](#delivery-3)] |
...
- a revision with a deliberately failing migration leaves the serving revision unchanged [[delivery-8](#delivery-8)];
- a revision whose migration succeeds but whose activation then fails leaves the previous revision serving correctly against the migrated schema [[delivery-4](#delivery-4)], [[delivery-9](#delivery-9)];
- a passing revision serves after its migrations apply [[delivery-4](#delivery-4)];
- the deployed revision reports the exact commit that produced it [[delivery-6](#delivery-6)];
- the preview's backing-service endpoints are disjoint from production's [[delivery-5](#delivery-5)].

### Hygiene Coverage

#### delivery-12

The audit suite shall assert a repository scan finds no secret values and that the example environment file lists every required variable name with no values [[delivery-7](#delivery-7)].
