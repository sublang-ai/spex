...
Where a fresh production deployment is serving [[delivery-4](ops/delivery.md#delivery-4)] with an initial-admin account configured [[access-control-1](identity/access-control.md#access-control-1)] and no user record exists, when that account completes GitHub sign-in [[github-login-2](identity/github-login.md#github-login-2)] from the guard's redirect [[access-control-4](identity/access-control.md#access-control-4)] off its course-manager request, the site shall treat it as admin from the first response after sign-in: the header carries the Admin entry [[web-shell-2](site/web-shell.md#web-shell-2)] and the course manager — the guarded target — opens, with no manual role-assignment step anywhere in the path.

### admin-bootstrap-2

While no course exists, when the admin opens the course manager, the path from an empty site to its first course shall be one action deep: creation is the manager's primary action [[course-catalog-7](catalog/course-catalog.md#course-catalog-7)].
