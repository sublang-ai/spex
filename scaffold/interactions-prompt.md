...
- Each interaction file needs an H1 with its own ALLCAPS short form and the sections of META-34: ## Intent, then ## Binding and/or ## Scenario, then ## Tests; test items keep a `Verifies:` line citing the same-file items they execute plus the package items they check (e.g. `[AUTH-3](../packages/auth.md#auth-3)`).
