...
- Where a package names an abstract subject or open slot (e.g. "the deployment's media provider"), add a binding item wiring it to another package's items or to a concrete service (META-35, META-36); a seam no product user observes is a supply binding, tested by inspecting the deployment rather than by a user journey.
- A binding item is static — no trigger clause: its preconditions cite the client items (or name the deployment surface), and its shall clause cites the supplier's External Behavior, names the selected service, or states the installation's own rule over cited External inputs (META-36).
