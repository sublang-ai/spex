...
- Migrate cross-package files to @specs/compositions/ and the fixed Intent/Binding?/Scenario?/Verification/References? grammar. Add explicit endpoint roles and scope to Bindings. In normative items, put every material behavior, installed-handoff, and verification citation inline beside the exact causal phrase or assertion; remove `Composes:`, `Bindings:`, and `Verifies:` trace lines and every supporting or incidental item link.
...
- Finish by validating exact links and every applicable META rule with format-compatible checks; never restore removed trace lines to satisfy an older checker.
