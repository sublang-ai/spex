...
- Migrate cross-package files to @specs/compositions/ and the fixed Intent/Binding?/Scenario?/Verification/References? grammar. In each Binding, state applicability and client givens in its `Where` clause and the installed provision in its shall clause, using exact inline endpoint citations or a named service with its selecting decision. Put every material behavior, installed-handoff, and verification citation inline beside the exact causal phrase or assertion; remove endpoint metadata, `Composes:`, `Bindings:`, and `Verifies:` trace lines, and every supporting or incidental item link.
