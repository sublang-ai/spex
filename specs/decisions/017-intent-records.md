...
- `specs/iterations/` becomes `specs/intents/`, and an IR is an **intent record** [[meta-1](../meta.md#meta-1)].
  The acronym, existing item IDs, and `<NNN>-<kebab-case>.md` naming are unchanged, so existing `IR-<N>` references stay valid wherever they were already allowed — commits and issues, never another spec [[meta-18](../meta.md#meta-18)].
...
- Intent records stay on the management plane: no other spec cites an IR — not the map, not a peer IR [[meta-18](../meta.md#meta-18)] — so behavior truth remains reimplementable from spec items alone and each IR stays independently disposable.
