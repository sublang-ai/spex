...
- Localization adds meta-29 (declaring `en`) to the English base once as a language-neutral hook; later languages add overlay files only and do not edit the English base.
...
- A kept-English item must cite a localized surface form by item ID instead of hard-coding it (for example, the intent section required by [[meta-15](../meta.md#meta-15)]); this keeps byte-identical parity valid across overlays.
...
| --- | --- | --- |
| [[meta-15](../meta.md#meta-15)] | `## Intent` heading | appears in every item file |
...
| [[meta-9](../meta.md#meta-9)] | GEARS pattern + clause keywords | the keywords every item is written in |
| [[meta-10](../meta.md#meta-10)] | GWT-to-GEARS mapping | companion to meta-9 |
| [[meta-27](../meta.md#meta-27)] | `## References` section convention | heading in specs that cite sources |
| [[meta-29](../meta.md#meta-29)] (new) | authoring-language declaration | see [Language as a spec item](#language-as-a-spec-item) |
...
- Localized meta-9 and meta-10 draw on the published Chinese GEARS reference [[1]], which defines the clause meanings (静态前置条件 / 状态前置条件 / 触发 / 所要求的行为) and the GWT mapping; this DR does not treat the reference as fixing the exact Chinese clause keywords.
- Exact Chinese surface wording is fixed in localized `meta.md` (meta-9), not by this DR, so it stays anchored to the cited reference without a separate glossary.

### Language as a spec item

- meta-29 is a new framework item: specs shall be authored in the language it declares.
...
- Agents read `meta.md` — reinforced by the `agent-specs.txt` pointer and `map.md` — before authoring: `--lang zh` writes a localized `meta.md` carrying meta-29, so new specs are authored in Chinese.
- Before overwriting framework files, `--update` reads the existing meta-29 declaration, falls back to `en` when no declaration is present (a missing or pre-meta-29 `meta.md`), and rejects `--lang`; deliberate language switching is out of scope for the first cut.
...
- The drift guard tests three things: completeness — every base `meta.md` item appears in each overlay, so new base items cannot be silently dropped; kept-English parity — untranslated items are byte-identical between the base and each overlay, catching accidental edits; translation freshness — each translated overlay item is pinned to the canonical hash of its English source item, so the test fails when the source changes without the translation being refreshed (for example, meta-9 changes in English but not in the `zh` overlay).
...
- Exact Chinese clause renderings live in localized `meta.md` (meta-9), informed by the cited reference [[1]]; this DR does not pin individual renderings such as `shall`.
...
- The translated set is a judgment call and may expand later (for example, meta-28, the DR-000 item-syntax section, or seed examples).
