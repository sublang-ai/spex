...
- Migration is mechanical, not manual: `spex scaffold --update` merges legacy trees with a real Markdown parser (byte-faithful slicing, heading demotion, reference renumbering), rewrites citations across `specs/`, and restructures a customized `map.md` [[scaffold-39](../packages/scaffold.md#scaffold-39)]–[[scaffold-42](../packages/scaffold.md#scaffold-42)]. Agent prompts cover what a tool cannot infer: intent reconciliation and interactions content.
...
- The desktop app's spec view still parses the legacy layout (the spec-view package); adapting `packages/core`'s parser, the protocol group triple, and the view is deferred to a follow-up iteration — until then the Specs tab cannot render this repo's own migrated tree.
