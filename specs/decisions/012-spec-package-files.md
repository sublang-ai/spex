...
- Two item directories replace the three folders ([DR-000](000-spec-structure-format.md)):
  ...
  - `specs/interactions/` — cross-package behaviors and scenarios, named after the behavior (never package-name concatenations), holding the integration/acceptance tests that span packages ([META-31](../meta.md#meta-31)).
...
- META items evolve in place: META-1/9/10/20/21 were rewritten for the new structure under their existing IDs — following the precedent of the `specs/items/` flattening — because their roles (layout, package composition, naming, Verifies, test scope) are unchanged; anchors and citations stay valid. New rules got new IDs (META-28, META-31). DR-000 likewise evolved in place, as the framework-owned living record of the current structure.
