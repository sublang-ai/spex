...
- The desktop app's spec view still parses the legacy layout (SPECV package); adapting `packages/core`'s parser, the protocol group triple, and the view is deferred to a follow-up iteration (IR-017) — until then the Specs tab cannot render this repo's own migrated tree.
