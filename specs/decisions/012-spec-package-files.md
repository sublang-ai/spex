...
- Two item directories replace the three folders ([DR-000](000-spec-structure-format.md)):
  ...
  - `specs/interactions/` — cross-package behaviors and scenarios, named after the behavior (never package-name concatenations), holding the integration/acceptance tests that span packages ([META-30](../meta.md#meta-30)).
...
- The desktop app's spec view still parses the legacy layout (SPECV package); adapting `packages/core`'s parser, the protocol group triple, and the view is deferred to a follow-up iteration ([IR-017](../iterations/017-spec-structure-refactor.md)) — until then the Specs tab cannot render this repo's own migrated tree.
...
- The spec view of the desktop app lags the new layout until IR-017 completes.
