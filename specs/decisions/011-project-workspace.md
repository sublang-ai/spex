...
- **Items.** Per-file document order with the files' `##` section headings preserved as sub-group labels — never sorted by ID ([[meta-18](../meta.md#meta-18)] makes numbering non-positional). Default row: ID chip + group tag + first sentence, one line; expanding renders the full body as markdown (tables, lists, code; horizontal overflow contained). Expand-all per package. Every ID chip is click-to-copy with a confirmation tick.
