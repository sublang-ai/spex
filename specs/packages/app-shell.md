...
Where the live-smoke handshake variable names a file
([DR-020](../decisions/020-desktop-live-smoke.md)), when the
desktop app starts, the desktop package shall redirect its
user-data directory to the smoke-provided location before opening
any store, start the core as usual, and write the core's socket
address to the named file only after the core is listening; where
the acceptance variable is also set, the app shall refuse to
launch naming the conflict, and where neither variable is set the
