...
// "Verifies" plus citations and separators. meta-16 wants each
    ...
    for (let line = item.bodyStart; line < item.bodyEnd; line += 1) {
      ...
      if (
        ...
        report(
          ...
          "error",
          "cite/detached",
          `item ${item.id} carries a detached "Verifies …" sentence; weave each citation into the assertion it supports (meta-16)`,
        );
      }
    }
...
function lintRecords(ctx: LintContext): void {
  // A record's ID is its filename's leading number (meta-22), so
    ...
    if (digits !== undefined) {
      ...
      if (first === undefined) {
        ...
        report(
          ...
          "error",
          "record/duplicate-id",
          `record ID ${id} is already defined by ${first} (meta-22)`,
        );
      }
    }
