...
- **Duplicate record numbers are caught instead of colliding silently.** Record ids form from the filename's leading number, so two differently named files could both become `IR-001`: `spex lint` now errors (`record/duplicate-id`) on a repeated number per record kind — `decisions/` for DRs, `intents/` together with legacy `iterations/` for IRs — and `spex scaffold --update` keeps an id-colliding legacy record in place, reporting the conflict against the id holder instead of migrating it into the collision ([META-1](../../specs/meta.md#meta-1), [SCAF-51](../../specs/packages/scaffold.md#scaf-51)).
