...
- **`spex lint` warns on multi-sentence items.** An item is one GEARS pattern — at most one trigger governing one shall clause — whose `<behavior>` may attach structured content: an ordered list, table, fenced block, or chart ([META-42](../../specs/meta.md#meta-42)). Prose carrying more than one sentence draws an advisory `item/sentence` warning (Binding items keep their `binding/sentence` error); the sentence count is a proxy for the pattern rule, so a multi-trigger single sentence is review's to catch, and the warning stays advisory while existing trees reduce their backlog. META-21's bundled test rules split into [META-38..41](../../specs/meta.md#meta-38) and lint messages now cite the split items.
